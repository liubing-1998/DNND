...
if args.network == 'grid':
    ...
    print("Choose graph: " + args.network)
    G = nx.barabasi_albert_graph(n, 4, seed=seed)  # 5
    G = networkx_reorder_nodes(G, args.layout)
    A = torch.FloatTensor(nx.to_numpy_array(G))
...
if __name__ == '__main__':
    ...
    model = DNND(activation_function=args.activation_function, input_size=input_size, hidden_A_list=hidden_A_list, hidden_list=hidden_list, A=A, rtol=args.rtol,
                 atol=args.atol, method=args.method)
